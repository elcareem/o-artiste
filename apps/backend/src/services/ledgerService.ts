...
async function record(
  tx: PrismaTx,
  { bookingId, entryType, party, amountKobo, description, offsetsEntryId }: LedgerEntryInput
...
function assertTransactionClient(tx: PrismaTx): void {
  ...
  // Cast, because the type deliberately omits `$transaction` — that omission is
  // exactly how a transaction client differs from the base one, and this check
  // is what turns the difference into an enforced rule at runtime.
...
async function appendAll(tx: PrismaTx, bookingId: string, legs: LedgerLeg[]): Promise<LedgerEntryRow[]> {
  const written = [];
 ...
 */
async function recordFunding(tx: PrismaTx, booking: BookingRow) {
 ...
 */
async function recordRelease(tx: PrismaTx, booking: BookingRow) {
 ...
 */
async function recordClientCancellation(
  tx: PrismaTx,
  booking: BookingRow,
 ...
 * booking pays it off.
 */
async function recordArtistCancellation(tx: PrismaTx, booking: BookingRow) {
 ...
 */
async function recordFeeLiabilitySettlement(tx: PrismaTx, bookingId: string, settledKobo: Kobo) {
...
async function recordCorrection(
  tx: PrismaTx,
  { offsetsEntryId, reason }: { offsetsEntryId: string; reason?: string }
) {
  assertTransactionClient(tx);
 ...
 */
async function reconcile(bookingId: string, client: PrismaLike = prisma): Promise<Reconciliation> {
  const entries = await client.ledgerEntry.findMany({
    where: { bookingId },
    orderBy: { createdAt: 'asc' },
  });

  const byParty: Record<LedgerParty, Kobo> = { CLIENT: 0, ARTIST: 0, PLATFORM: 0, PROVIDER: 0 };
  let sumKobo = 0;
 ...
 */
async function assertBalanced(bookingId: string, client: PrismaLike = prisma): Promise<Reconciliation> {
...
function completionFor(booking: BookingRow): CompletionBreakdown {

...
function applyBps(baseKobo: Kobo, bps: Bps): Kobo {
...
function moneyInFee(amountKobo: Kobo): Kobo {
...
function moneyOutFee(payoutKobo: Kobo): Kobo {
...
function computeClientCancellation({
  ...
  refundIncursMoneyOut?: boolean;
}): ClientCancellationBreakdown {
  assertInteger(amountKobo, 'amountKobo');
 ...
 */
function applyFeeLiabilities({
  payoutKobo,
  liabilitiesKobo,
}: {
  payoutKobo: Kobo;
  liabilitiesKobo: Kobo;
}): LiabilitySettlement {
  assertInteger(payoutKobo, 'payoutKobo');
...
function assertBpsPairSums(a: Bps, b: Bps): void {

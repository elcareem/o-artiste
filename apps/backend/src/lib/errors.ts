...
class AppError extends Error {
  ...
  status: number;
  /** Marks this as a deliberate, user-facing error rather than a bug. */
  expected: boolean;

  /**
   * @param status HTTP status
   * @param message User-facing copy. Name what went wrong and, where
...
function notFoundHandler(_req: Req, _res: Res, next: Next) {
...
function errorHandler(err: unknown, _req: Req, res: Res, _next: Next) {

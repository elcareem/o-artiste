 ...
 */
const SELF_REGISTERABLE_ROLES = Object.freeze(['CLIENT', 'ARTIST']);
...
function signToken(user, expiresIn = process.env.JWT_EXPIRES_IN || DEFAULT_EXPIRY) {
  return jwt.sign({ sub: user.id, role: user.role }, jwtSecret(), { expiresIn });
}

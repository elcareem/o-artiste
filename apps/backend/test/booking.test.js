...
const { prisma, hasDatabase, ready } = require('./db')('booking');
...
const booking = require('../src/services/bookingService');
const commission = require('../src/services/commissionService');
const tiers = require('../src/services/cancellationTierService');
...
async function makeUser(role, { verified = true, standing = 'GOOD' } = {}) {
...
describe('changing the tier table after creation does not alter that booking', async () => {
  ...
  const dayOf = snapshot.find((t) => t.minDaysBefore === 0);
  ...
  assert.equal(
    reread.cancellationTiersSnapshot.find((t) => t.minDaysBefore === 0).clientRefundBps,
    1500,
...
describe('a booking for a past event date returns 400', async () => {
  await withServer(async (server) => {
    ...
    for (const when of [new Date(Date.now() - 86400000), new Date(Date.now() - 1000)]) {
      ...
      assert.match((await res.json()).error, /future/i);
    }
...
describe('suspended clients and unverified or suspended artists are refused', async () => {
  await withServer(async (server) => {
    ...
    await assert.rejects(
      ...
      (err) => err.status === 403 && /suspended/i.test(err.message)
    );
...
describe('the payout preview is computed from the snapshot, not live config', async () => {
  await withServer(async (server) => {
    ...
    const { computeCompletion } = require('../src/services/feeService');

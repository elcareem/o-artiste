...
const { prisma, hasDatabase, ready } = require('./db')('release');
...
describe('no route handler or job calls the provider release method directly', async () => {
  ...
  const hits = execFileSync(
    'bash',
    ['-c', `grep -rnE 'escrowpay\\.(release|refund)' "${root}/apps/backend/src" | grep -vE 'services/escrowService\\.js|lib/escrowpay\\.js' || true`],
    { encoding: 'utf8' }
  ).trim();
